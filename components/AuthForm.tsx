...
import { handleEmailAuth } from "@/app/actions/email-auth"
import { handleDiscordAuth } from "@/app/actions/discord-auth"
...
export default function AuthForm({
  ...
  const [state, formAction, isPending] = useActionState(handleEmailAuth, {})
  ...
  return (
    <>
      ...
      <div className="flex bg-[#0a0a1a]/50 rounded-xl p-1.5 mb-8 border border-cyan-500/20">
        <form action={() => router.push("/?view=signin")} className="flex-1">
          <Button
            type="submit"
            className={`w-full ${
              !isSignUp
        ...
        <form action={() => router.push("/?view=signup")} className="flex-1">
          <Button
            type="submit"
            className={`w-full ${
              isSignUp
      ...
      <form action={formAction} className="space-y-4">
        ...
        <Button
          type="submit"
          disabled={isPending}
          className="w-full bg-cyan-500 hover:bg-cyan-600 text-black font-bold py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
